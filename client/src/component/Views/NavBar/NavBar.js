...
function NavBar(props) {
    let navBarContext = [
        ...
        { value: "logout",    text: "로그아웃", bgColor: "#C0C0C0" }
    ]
        ...
        if (!DisplayBottom){
            let navBarBottom = navBarContext.map(item => {
                if(categoryName === item.value){
                    ...
                    (categoryName === 'logout') ? { value : item.value, text: '나가기', bgColor: item.bgColor} :
    ...
    const newMemoHandler = (e) => {
        if(e.currentTarget.innerText === '나가기')
            props.logoutHandler();
        else if (e.currentTarget.innerText !== '새 메모')
            return false
        else props.newMemoHandler()
    }
